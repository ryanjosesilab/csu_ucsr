...
import { FaCheck, FaTimes, FaPrint, FaChevronDown, FaChevronUp, FaFileAlt, FaDownload, FaFolderOpen } from 'react-icons/fa';
...
export default function DLCAdminPage() {
  ...
  const fetchRequests = async () => {
    const { data, error } = await supabase
      ...
      .is('request_number', null) // <--- ADD THIS LINE: Only fetch forms that haven't been printed yet
...
{req.pdf_url ? (
  ...
  <div className="w-full h-32 bg-gray-50 flex items-center justify-center rounded border border-dashed border-gray-300 mb-6 text-gray-500 italic">
    ...
    onClick={(e) => {
      e.stopPropagation(); 
      // 1. Open the print page
      window.open(`/print-dlc/${req.id}`, '_blank');
      // 2. Immediately remove it from this screen's local list
      setRequests(prevRequests => prevRequests.filter(r => r.id !== req.id));
    }}
